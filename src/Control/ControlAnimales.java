    ...
    public Object ObtenerDatosAnimalesPesables(String IDFINCA, String IDTIPOANIMAL, String FECHA) {
        try {
            String consulta = "SELECT\n"
                    ...
                    + "IFNULL((SELECT '*' FROM pesaje m WHERE m.estado='Activo' and m.id_animal=a.id ),'') AS EST,\n"
                    + "IFNULL((SELECT id FROM pesaje m WHERE m.estado='Activo' AND m.id_animal=a.id ),'') AS ID_PESAJE,\n"
                    + "IFNULL((SELECT notas FROM pesaje m WHERE m.estado='Activo' AND m.id_animal=a.id) ,'') AS NOTAS,\n"
                    + "IFNULL((SELECT estado FROM pesaje m WHERE m.estado='Activo' AND m.id_animal=a.id) ,'') AS ESTADO,\n"
                    + "IFNULL((SELECT DATE_FORMAT(fecha_pesado,'%d/%m/%Y') FROM pesaje m WHERE m.estado='Activo' AND m.id_animal=a.id) ,'') AS FECHAPESADO,\n"
